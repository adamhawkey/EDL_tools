...
# -*- coding: utf-8 -*-
"""
version: GT_prep_0.5.py
"""

# This script is very specific to the show Good Trouble.
# Versioning up to 0.5 to split lutname on the _VEN delimiter.
# Versioning up to 0.4 as we begin season 4 in November 2021.
...
for clip in timeline.each_clip():
    ...
    for comment in comments:
        if 'REEL: GT_' in comment:
            ...
            lut_string_split = re.split(r'_VEN', lut_string)
            lut_string_chars = re.split(r'[^A-Za-z0-9_]+', lut_string_split[0])
            lut_root = lut_string_chars[0]
            if args.verbose:
                print('{0} becomes {1}_{2}.{4}'.format(lut_string, lut_root, lut_space, lut_version, lut_ext))
